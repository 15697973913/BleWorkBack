package com.zj.zhijue.view.recyclerview;
...
public class GridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mBuilder.isShowHeadDivider && itemPosition < spanCount) {

            if (isLastColum(parent, itemPosition, spanCount, childCount)) {
                outRect.set(0, mBuilder.dividerHorSize, 0, mBuilder.dividerHorSize);
            } else {
                outRect.set(0, mBuilder.dividerHorSize, mBuilder.dividerHorSize, mBuilder.dividerHorSize);
            }
        ...
        if (!mBuilder.isShowLastDivider) {
            /**
             * 处理不需要显示最后一行的分割线的情况，
             * 处理每行只有一列的情况，最后一行不需要分割线的情况
             */
            if (spanCount == 1 &&  childCount > 0 && childCount - 1 == itemPosition) {
                    bottom = 0;
            }
        }
