...
public class WheelView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(centerX, height / 2, r1, p);
        if (pressFlag >= CLICK_RIGHT_DOWN && pressFlag <= CLICK_TOP_DOWN) {
            p.setColor(innerClr);
            canvas.drawArc(new RectF(intervalX, 0, height + intervalX, height), pressFlag * 90 - 45, 90, true, p);
        }
        ...
        p.setColor(borderClr);
        canvas.drawLine(r1 - r1 / factor + intervalX, r1 - r1 / factor, r1 + r1 / factor + intervalX, r1 + r1 / factor, p);
        canvas.drawLine(r1 - r1 / factor + intervalX, r1 + r1 / factor, r1 + r1 / factor + intervalX, r1 - r1 / factor, p);
        p.setColor(arrowClr);
        p.setStrokeJoin(Paint.Join.MITER);
        canvas.drawLine(height / 8 + 12 * density + intervalX, height / 2 - 12 * density, height / 8 + intervalX, height / 2, p);
        canvas.drawLine(height / 8 + intervalX, height / 2 , height / 8 + 12 * density + intervalX, height / 2 + 12 * density, p);
        canvas.drawLine(height * 7 / 8 - 12 * density + intervalX, height / 2 - 12 * density, height * 7 / 8 + intervalX, height / 2, p);
        canvas.drawLine(height * 7 / 8 - 12 * density + intervalX, height / 2 + 12 * density, height * 7 / 8 + intervalX, height / 2, p);
        canvas.drawLine(height / 2 - 12 * density + intervalX, height / 8 + 12 * density, height / 2 + intervalX, height / 8, p);
        canvas.drawLine(height / 2 + 12 * density + intervalX, height / 8 + 12 * density, height / 2 + intervalX, height / 8, p);
        canvas.drawLine(height / 2 - 12 * density + intervalX, height * 7 / 8 - 12 * density, height / 2 + intervalX, height * 7 / 8, p);
        canvas.drawLine(height / 2 + 12 * density + intervalX, height * 7 / 8 - 12 * density, height / 2 + intervalX, height * 7 / 8, p);
        p.setColor(innerClr);
        p.setStyle(Paint.Style.FILL);
        canvas.drawCircle(centerX,height / 2, r2, p);
        p.setColor(borderClr);
        p.setStyle(Paint.Style.STROKE);
        canvas.drawCircle(centerX, height / 2, r2, p);
