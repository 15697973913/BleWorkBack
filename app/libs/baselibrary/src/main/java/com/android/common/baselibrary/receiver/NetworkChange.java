package com.android.common.baselibrary.receiver;
...
public class NetworkChange extends BroadcastReceiver {
    ...
    public void registerNetReceiver() {
        ...
        BaseApplication.getInstance().registerReceiver(NetworkChange.getInstance(), filter);
        isRegistered = true;
    }

    public void unRegisteredNetReceiver() {
        if (isRegistered) {
            try {
                BaseApplication.getInstance().unregisterReceiver(NetworkChange.getInstance());
            } catch (Exception e) {
                e.printStackTrace();
            }
            isRegistered = false;
    ...
    private static class Innner {
        public final static NetworkChange single = new NetworkChange();
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if ((null != mobNetInfo && !mobNetInfo.isConnected()) && (null != wifiNetInfo && !wifiNetInfo.isConnected())) {
            Log.i("通知", "网络不可以用");
            //ToastUtil.showShortToast("网络不可以用");
            setChange(none);
        } else if (null != mobNetInfo && mobNetInfo.isConnected()) {
            Log.i("通知", "仅移动网络可用");
            //ToastUtil.showShortToast("仅移动网络可用");
            ...
            Log.i("通知", "Wifi网络可用");
            //ToastUtil.showShortToast("Wifi网络可用");
            LocalBroadcastManager.getInstance(BaseApplication.getInstance()).sendBroadcast(new Intent(BaseApplication.getInstance().getPackageName() + BaseConstantString.NET_ACTION));
            setChange(wifi);
        }
    }
