...
public class EyeAstigmatismDegreeInputFilter implements InputFilter {
    Pattern mPattern;

    private static final String MINUS = "-";

    public EyeAstigmatismDegreeInputFilter() {
    ...
    public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
        ...
        if (destText.trim().length() >=2) {
            boolean biggerMAX = Math.abs(Integer.parseInt(destText + source)) > 100;
            if (biggerMAX) {
                return "";
            }
        }
