...
public class ToastUtil {
    ...
    //是否在固件升级中
    public static boolean isDfuUpdate = false;
    ...
    public static void showShort(@NonNull CharSequence text) {
        if (text != null && text.toString().contains("HTTP 401")) {
            text = "您的账号已在其它设备登录，请重新登录";
        }
    ...
    public static void showLongWithMsgColor(@NonNull final CharSequence text, int textColor) {
        showWithTextColor(text, Toast.LENGTH_LONG, textColor);
    }
    ...
    private static void show(CharSequence text, final int duration) {
        MLog.e("============= " + text);
        //简单屏蔽提示
        if (text.toString().contains("Failed to connect")
                || text.toString().contains("Unable to resolve host")) {
            text = "手机网络异常，请检查后再试";
        }

        if (TextUtils.equals(text, "closed")) {
            return;
        }
        ...
        HANDLER.post(new Runnable() {
            ...
            public void run() {
                cancel();
                sToast = Toast.makeText(BaseApplication.getInstance(), null, duration);
                sToast.setText(finalText);
    ...
    private static void showWithTextColor(final CharSequence text, final int duration, final int textColor) {
        HANDLER.post(new Runnable() {
            @SuppressLint("ShowToast")
            @Override
            public void run() {
    ...
    private static void showToast() {
        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.N_MR1) {
            try {
                ...
                field.set(sToast.getView(), new ToastUtil.ApplicationContextWrapperForApi25());
    ...
    private static final class ApplicationContextWrapperForApi25 extends ContextWrapper {
        ...
        public Object getSystemService(@NonNull String name) {
            if (Context.WINDOW_SERVICE.equals(name)) {
                ...
                return new ToastUtil.ApplicationContextWrapperForApi25.WindowManagerWrapper(
