...
public class BackgroundPowerSaver implements Application.ActivityLifecycleCallbacks {
    ...
    public BackgroundPowerSaver(Context context) {
        if (android.os.Build.VERSION.SDK_INT < 18) {
            ViseLog.w("BackgroundPowerSaver requires API 18 or higher.");
            return;
        }
    ...
    public void onActivityResumed(Activity activity) {
        ...
        if (activeActivityCount < 1) {
            ViseLog.d("reset active activity count on resume.  It was " + activeActivityCount);
            activeActivityCount = 1;
        }
        BluetoothScanManager.getInstance(mContext).setBackgroundMode(false);
        ViseLog.d("activity resumed: "+activity+" active activities: "+activeActivityCount);
    }

    @Override
    public void onActivityPaused(Activity activity) {
        activeActivityCount--;
        ViseLog.d("activity paused: "+activity+" active activities: "+activeActivityCount);
        if (activeActivityCount < 1) {
            ViseLog.d("setting background mode");
