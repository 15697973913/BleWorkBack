...
import com.vise.baseble.newscantype.scan.bluetoothcompat.ScanResultCompat;
 ...
 * <li>当前扫描状态</li>
 * </ol>
 * 注意：回调不在主线程中执行，需要自己在主线程中处理回调（{@link ）,
...
public final class BluetoothScanManager {
    ...
    public static BluetoothScanManager getInstance(Context context){
        if (INSTANCE == null){
            synchronized (obj){
                if (INSTANCE == null){
                    ViseLog.d("BluetoothScanManager instance creation");
    ...
    private ScanOverListener mScanOverListener = new ScanOverListener() {
        @Override
    ...
    private ScanCallbackCompat mScanCallbackCompat = new ScanCallbackCompat() {
        @Override
        public void onScanResult(int callbackType, ScanResultCompat result) {
            super.onScanResult(callbackType, result);
            if (null != mBluetoothLeDeviceStore) {
                mBluetoothLeDeviceStore.addDevice(result.getLeDevice());
            }
            if (null != mIScanCallback) {
                mIScanCallback.onDeviceFound(result.getLeDevice());
            }
        }
    ...
    private BluetoothLeDeviceStore mBluetoothLeDeviceStore = null;
    private IScanCallback mIScanCallback = null;
    public void startScanWithNewDef(Context context, BluetoothLeDeviceStore bluetoothLeDeviceStore, IScanCallback iScanCallback) {
        ...
        scanManager.setScanOverListener(mScanOverListener);
        scanManager.setScanCallbackCompat(mScanCallbackCompat);
        startScanNow();
    }
