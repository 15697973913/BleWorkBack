...
public class ViseBle {
    ...
    private SingleFilterScanCallback singleFilterScanCallback = null;
    ...
    public void startScan(ScanCallback scanCallback) {
        ...
        scanCallback.setScan(true).scan(context);
    }
    ...
    public void stopScan(ScanCallback scanCallback) {
        Log.e("ScanCallback", "stopScan" + scanCallback);
        if (scanCallback == null) {
            throw new IllegalArgumentException("this ScanCallback is Null!");
        }
        scanCallback.setScan(false).removeHandlerMsg().scan(context);
    }

    public void stopScanByMac() {
        if (null != singleFilterScanCallback) {
            singleFilterScanCallback.setScan(false).removeHandlerMsg().scan(context);
        }
    }

    public boolean isScanningByMac() {
        if (null != singleFilterScanCallback) {
            return singleFilterScanCallback.isScanning();
        }
        return false;
    }
    ...
    public void connectByMac(String mac, final IConnectCallback connectCallback) {
        ...
        singleFilterScanCallback = new SingleFilterScanCallback(new IScanCallback() {
            @Override
            ...
            public void onScanFinish(final BluetoothLeDeviceStore bluetoothLeDeviceStore) {
                if (bluetoothLeDeviceStore.getDeviceList().size() > 0) {
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            List<BluetoothLeDevice> leDeviceList = bluetoothLeDeviceStore.getDeviceList();
                            if (null != leDeviceList && leDeviceList.size() > 0) {
                                BluetoothLeDevice bluetoothLeDevice = leDeviceList.get(0);
                                connectCallback.foundDeviceByMac(bluetoothLeDevice);
                    ...
                    /**
                     * 区分的目的，根据本地是否保存的有mac地址，来确定是否需要重新扫描
                     */
                    connectCallback.onConnectTimeoutOrFailByMac();
                }
            }
        ...
        startScan(singleFilterScanCallback.setDeviceMac(mac));
    }
